...
def loop_once(robot_at, history, room_array, all_waypoints, all_types, adjacency_matrix, action=None):
    ...
    # Choose new waypoint
    if action is None:
        moves = possible_moves(all_waypoints, adjacency_matrix, robot_at)
        robot_at = choose_next_move(possible_moves=moves, history=history)
        #print(f"Possible moves: \n{moves} \n {all_types[moves]} \n {room_array[moves]}")
        #print(f"Moving to waypoint {robot_at}, {all_types[robot_at]}, {room_array[robot_at]}")
    else:
        robot_at = action
        #print(f"Moving to waypoint {robot_at}, {all_types[robot_at]}, {room_array[robot_at]}")
